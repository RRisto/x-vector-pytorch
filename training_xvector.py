...
parser.add_argument('-training_filepath', type=str, default='meta_et_ru_fi/training.txt')
parser.add_argument('-testing_filepath', type=str, default='meta_et_ru_fi/testing.txt')
...
def train(dataloader_train, epoch):
    ...
    for i_batch, sample_batched in enumerate(dataloader_train):
        ...
        #### CE loss
        loss = loss_fun(pred_logits, labels.type(torch.LongTensor))
        loss.backward()
        optimizer.step()
...
if __name__ == '__main__':
    for epoch in range(args.num_epochs):
        ...
        end_time = time.time()
        print(f'training epoch time {round(end_time - start_time, 3)}')
